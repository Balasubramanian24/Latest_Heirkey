...
const OtherInstructions = () => {
  ...
  const questions = useAppSelector((state: any) => state.homeInstructions.questions['103'] || []);
  ...
  useEffect(() => {
    if (userInputs.length > 0 && !loading) {
      ...
      const userInput = userInputs.find((input: UserInput) => input.originalSubCategoryId === '103');
