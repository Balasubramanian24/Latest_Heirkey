...
const CeremonyLocation = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      // Set questions from JSON data (section 205B)
      if (funeralArrangementsData['205']) {
        // Map questions to ensure type compatibility
        const rawQuestions = funeralArrangementsData['205'].filter(q => q.sectionId === '205B');
      ...
      if (user && user.id) {
        try {
          ...
          const userInputs = await userInputService.getUserInputsBySubcategory(user.id, '2', '205B');
