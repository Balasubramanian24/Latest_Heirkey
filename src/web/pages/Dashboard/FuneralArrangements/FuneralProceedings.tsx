...
const FuneralProceedings = () => {
  ...
  const questions = useAppSelector((state: any) =>
    state.funeralArrangements.questions['205']?.filter((q: Question) => q.sectionId === '205E') || []
  );
  ...
  useEffect(() => {
    if (userInputs.length > 0 && !loading) {
      ...
      const userInput = userInputs.find((input: UserInput) => input.originalSubCategoryId === '205E');
  ...
  const handleSubmit = async (values: Record<string, string>, { setSubmitting }: FormikHelpers<Record<string, string>>) => {
    try {
      console.log('Saving proceedings details:', values);
      ...
      console.error('Error saving proceedings details:', error);
      setSubmitting(false);
