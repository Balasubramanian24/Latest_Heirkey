...
// Get the questions for clergy section
const clergyQuestionsRaw = (funeralArrangementsData["205"] || [])
  .filter(q => ["f8", "f9", "f10"].includes(q.id));

// Cast the questions to the proper type
const clergyQuestions = castToQuestionType(clergyQuestionsRaw);
...
export default function FuneralClergyPage() {
  ...
  const userInputs = useAppSelector((state) => selectUserInputsBySubcategoryId('205C')(state));
  ...
  const typedQuestions = clergyQuestions;
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              ...
              const formattedAnswersBySection = [{
                originalSectionId: '205C', // Store our original section ID for clergy
                isCompleted: true,
                answers
              }];
                ...
                try {
                  ...
                  await dispatch(updateUserInput({
                    ...
                    userData: {
                      ...
                      originalSubCategoryId: '205C', // Subcategory ID for clergy
              ...
              console.error('Error saving clergy information:', err);
